...
14. 环境配置记录：开发环境中必要的密钥、密码、邀请码、数值等校验数据，可记录在 `.m-work-flow/password.md` 中
15. 线上测试环境强制验收：本项目配置的线上环境均视为测试环境，不得因误认为生产环境而跳过、延后或缩减线上验证；涉及线上行为的功能，必须以线上环境实际跑通作为最终验收依据，本地单元测试、构建、Mock、抓包或接口自测均不能替代线上验证
16. 线上阻断记录：线上验证失败、无法访问、认证/浏览器/网络等依赖不可用时，必须在报告中如实记录 `FAIL` 或 `BLOCKED`、复现条件和未覆盖范围，不得将本地通过推断为线上通过
17. 测试账号与会话：在用户已授权的项目测试线上环境内，质量验证 Agent 可创建和使用临时测试账号/隔离会话完成多用户验证；凭据不得写入代码、提交、报告或长期存储，测试结束后应关闭或清理临时会话
...
本项目的线上部署地址属于测试线上环境。Agent 不得把它当作生产环境，也不得自行以"生产环境风险"为理由拒绝用户要求的线上测试。凡是影响用户流程、前后端衔接、WebSocket/实时状态、认证、部署资源或数据持久化的任务，交付前必须安排质量验证 Agent 在当前线上测试环境执行真实流程。

线上实际结果是该类任务的最终验收依据：

- 线上跑通并有可复核证据，才能标记 `PASS`；
- 线上失败必须标记 `FAIL`；
- 因环境、权限、浏览器、网络或依赖导致无法执行，必须标记 `BLOCKED`，并写明阻断原因、已做的替代验证和剩余风险；
- 本地测试、Mock、静态检查、构建成功或单客户端实验只能作为补充证据，不能替代线上多用户/真实交互验证；
- 用户已明确授权测试账号或会话时，允许使用隔离的临时账号完成线上验证；账号凭据只在运行时使用，不得落盘或进入报告。
...
1. **浏览器相关实践与测试**：必须使用 `agent-browser`
   ...
   - **Session 命名规范**：创建 Browser session 时必须以 `Agent-browser-` 为强制前缀，后接有意义的标识（如当前任务标识、线程名称等），格式为 `Agent-browser-<任务标识>`；禁止使用简单数字（1,2,3,4）或省略前缀，避免多个 Agent 并发使用时导致 session 冲突或误读幻觉；不同 Thread 需协同测试时，应互相说明具体 session 名称以便复用
